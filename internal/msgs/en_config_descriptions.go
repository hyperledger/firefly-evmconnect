...
var (
	...
	_ = ffc("config.connector.chainTrackingMode", "Chain tracking mode. `light`: fetches head block numbers only, does not download block details, disables block listener support, and confirmation results include only the confirmation count. `full`: fetches head block numbers and block details, maintains an in-memory partial chain, enables block listener support, and confirmation results include both confirmation count and block details.", "`light` or `full`")
)
